...
{
	WriteStruct_Pat(buf);
	SendPushData(buf,TS_PACKET_SIZE, 0);
...
//	int result = fwrite(lpszBuf,TS_PACKET_SIZE,1,pVideo_Audio_Ts_File);
	return HttpPusherPush(lpszBuf, TS_PACKET_SIZE, 0);
}
...
//		LOGE("FWRITE_before:%d,%d,%d",TSbuf[0],TSbuf[1],TSbuf[2]);
		//����д���ļ�
		int result = 0;
//		int result = fwrite(TSbuf,188,1,pVideo_Audio_Ts_File);                              /将一包数据写入文件
		SendPushData(TSbuf, TS_PACKET_SIZE, 0);
//		LOGE("FWRITE_after:%d,%d,%d",TSbuf[0],TSbuf[1],TSbuf[2]);
//		LOGE("write_result:%d",result);
...
int WriteBuf2TsFile(unsigned int framerate, int iStreamType, unsigned char *pData, int iDataSize, unsigned long lTimeStamp,int type)
{
	...
	Timestamp_audio = lTimeStamp*90;
	Timestamp_video = Timestamp_audio;
	if (0 == iStreamType)
	{
...
//				Timestamp_video += 1000 * 90 / framerate;
			...
			//LOGE("pts_video:%d",Timestamp_video);
		}
	}
