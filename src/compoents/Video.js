...
export default class extends PureComponent {
    ...
    onProgress = (data) => {
        ...
        if (this.isPlaying && currentTime >= this.state.duration) {
            this.onEnd();
        }
    };
    ...
    render() {
        ...
        const {playUri, style, handleBack, share, shiftTime = 0, title = '', onSection, actionBar, monitorState,onEnd} = this.props;
        return (
            <View style={[styles.container, style, this.isFull ? styles.fullScreen : {height: $.WIDTH * 9 / 16}]}>
                ...
                <Text style={[styles.showTime, !this._isSet && {opacity: 0}]}>
                    <Text
                        style={{color: $.COLORS.mainColor}}>{moment.utc((this._currentTime + shiftTime) * 1000).format("HH:mm:ss")}</Text>
                    /{moment.utc((duration + shiftTime) * 1000).format("HH:mm:ss")}
                </Text>
                <TipView data={this._currentBrightness}
                         type={['brightness-5', 'brightness-4', 'brightness-6', 'brightness-7']}
                         isSet={this._isSetBright}/>
                <TipView data={this._currentVolume} type={['volume-off', 'volume-mute', 'volume-down', 'volume-up']}
                         isSet={this._isSetVolumn}/>
                ...
                <View style={[styles.videoTop, {top: this.isFull ? $.STATUS_HEIGHT : 0}, !isShowBar && {top: -50}]}>
                    ...
                    {
                        monitorState ?
                            <View style={{width: 50, justifyContent: 'center', alignItems: 'center', backgroundColor:'rgba(0,0,0,0)'}}>
                                <Text style={{color: 'white', fontSize: 14}}>{monitorState}</Text>
                            </View>
                            :
                            null
                    }
